...
이 문서는 App Store Connect > App Privacy 입력용 체크리스트입니다. 현재 App Store 제출 빌드 기준으로 작성했습니다.
...
- 음성 인식, 발화자 라벨, 요약은 사용자의 기기에서 로컬 모델로 처리됩니다.
...
- 회의 녹음, 전사본, 요약본은 사용자의 기기에 로컬 저장됩니다.
- 개발자 서버로 업로드되지 않습니다.
...
App Store 제출용 빌드에서는 Hugging Face 토큰 입력 UI를 기본 노출하지 않습니다.

App Privacy Label에서는 `Identifiers` 또는 `Contact Info`를 선택하지 않습니다.

이유:

- App Store 제출 빌드에서는 기본 모델 다운로드 흐름이 외부 계정/토큰을 요구하지 않도록 구성되어 있습니다.
- 내부/개발 빌드에서 토큰 입력 UI를 켠 경우에도 토큰은 사용자가 모델 제공 사이트에서 받은 다운로드 권한 토큰입니다.
- 앱은 토큰을 모델 다운로드 요청에만 사용하며 앱 개발자 서버로 전송하지 않습니다.

### Privacy Manifest

`macos/Runner/PrivacyInfo.xcprivacy`를 포함합니다.

- Tracking: false
- Collected data types: none
- Required reason API: UserDefaults (`CA92.1`)

현재 빌드 산출물에서 `Contents/Resources/PrivacyInfo.xcprivacy` 포함을 확인했습니다.
