...
Local Minutes는 macOS용 로컬 AI 회의록 앱입니다. 기기 마이크 입력 기반 회의 녹음, 음성 인식, 발화자 라벨, 요약 생성을 사용자의 기기에서 처리하고, 회의 음성이나 전사본을 개발자 서버로 업로드하지 않는 것을 기본 원칙으로 합니다.

## 주요 기능

- 기기 마이크 입력 기반 회의 녹음과 WAV 저장
- 로컬 Whisper 기반 음성 인식
- 로컬 LLM 기반 회의 요약
...
- 첫 출시 버전은 온라인 회의 앱의 내부 시스템 오디오 직접 캡처를 제공하지 않습니다. 이 기능은 출시 후 업데이트 후보로 분리합니다.
