...
2. 열린 창에서 `Local Minutes.app`을 `Applications` 폴더로 드래그합니다.
...
```bash
xattr -cr "/Applications/Local Minutes.app"
```
...
```bash
mkdir -p ~/Library/Application\ Support/LocalMinutes/models
```
...
1. `/Applications/Local Minutes.app` 실행
...
### 전사가 되지 않음
- 시스템 환경설정 → 개인 정보 보호 → 마이크 → Local Minutes 허용 확인
