...
class ExportService {
  ...
  static String buildText(
    ...
    buf.writeln(sep);
    buf.writeln(tr('온디바이스 AI 회의록', 'On-device AI meeting minutes'));
    buf.writeln(sep);
    buf.writeln();
    ...
    final durStr = meeting.durationSeconds > 0
        ? ' | ${tr('소요시간', 'Duration')}: ${meeting.durationSeconds ~/ 60}${tr('분', 'm')} ${meeting.durationSeconds % 60}${tr('초', 's')}'
        : '';
    buf.writeln('${tr('날짜', 'Date')}: $dtStr$durStr');
    buf.writeln();

    if (summary != null) {
      // 참석자
      if (summary.participants.isNotEmpty) {
        buf.writeln('【${tr('참석자', 'Participants')}】');
      ...
      if (summary.keyDiscussions.isNotEmpty) {
        buf.writeln('【${tr('주요 논의', 'Key discussions')}】');
      ...
      if (summary.decisions.isNotEmpty) {
        buf.writeln('【${tr('결정 사항', 'Decisions')}】');
      ...
      try {
        final items = (jsonDecode(summary.actionItemsJson) as List)
            ...
            .toList();
        if (items.isNotEmpty) {
          buf.writeln('【${tr('액션 아이템', 'Action items')}】');
          for (final a in items) {
            buf.writeln(
              '• ${a.task}  [${tr('담당', 'Owner')}: ${_actionOwnerText(a)}] [${tr('기한', 'Due')}: ${_actionDeadlineText(a)}]',
            );
          }
          buf.writeln();
        }
      ...
      if (summary.openQuestions.isNotEmpty) {
        buf.writeln('【${tr('미해결 이슈', 'Open questions')}】');
  ...
  static String buildMarkdown(
    ...
    final dtStr =
        ...
        '${dt.minute.toString().padLeft(2, '0')}';
    final durStr = meeting.durationSeconds > 0
        ? '${meeting.durationSeconds ~/ 60}${tr('분', 'm')} ${meeting.durationSeconds % 60}${tr('초', 's')}'
        : '';

    buf.writeln('# ${_markdownText(meeting.title)}');
    buf.writeln();
    buf.writeln('- ${tr('날짜', 'Date')}: $dtStr');
    if (durStr.isNotEmpty) buf.writeln('- ${tr('소요 시간', 'Duration')}: $durStr');
    buf.writeln('- ${tr('생성', 'Generated by')}: Local Minutes ${tr('온디바이스 AI 회의록', 'on-device AI meeting minutes')}');
    buf.writeln();

    if (summary == null) {
      buf.writeln('> ${tr('요약이 아직 생성되지 않았습니다.', 'No summary has been generated yet.')}');
      buf.writeln();
    } else {
      if (summary.participants.isNotEmpty) {
        buf.writeln('## ${tr('참석자', 'Participants')}');
      ...
      _writeMarkdownList(buf, tr('주요 논의', 'Key discussions'), summary.keyDiscussions);
      _writeMarkdownList(buf, tr('결정 사항', 'Decisions'), summary.decisions);
      _writeMarkdownActionItems(buf, summary);
      _writeMarkdownList(buf, tr('미해결 이슈', 'Open questions'), summary.openQuestions);
    }
  ...
  static String buildNotionMarkdown(Meeting meeting, Summary? summary) {
    ...
    if (meeting.durationSeconds > 0) {
      buf.writeln(
        '- ${tr('소요 시간', 'Duration')}: ${meeting.durationSeconds ~/ 60}${tr('분', 'm')} ${meeting.durationSeconds % 60}${tr('초', 's')}',
      );
    }
    buf.writeln();
  ...
  static String buildBusinessReportMarkdown(Meeting meeting, Summary? summary) {
    ...
    buf.writeln('# ${_markdownText(meeting.title)} ${tr('보고', 'Report')}');
    buf.writeln();
    buf.writeln('| ${tr('항목', 'Item')} | ${tr('내용', 'Details')} |');
    buf.writeln('| --- | --- |');
    buf.writeln('| ${tr('일자', 'Date')} | $dtStr |');
    if (summary?.participants.isNotEmpty == true) {
      buf.writeln(
        '| ${tr('참석자', 'Participants')} | ${_markdownTableCell(summary!.participants.join(', '))} |',
      );
    }
    buf.writeln();
    ...
    _writeMarkdownList(buf, tr('핵심 논의', 'Key discussions'), summary.keyDiscussions.take(5).toList());
    _writeMarkdownList(buf, tr('결정 사항', 'Decisions'), summary.decisions);
    _writeNotionActionItems(buf, summary);
    _writeMarkdownList(buf, tr('리스크 / 확인 필요', 'Risks / Needs review'), summary.openQuestions);
  ...
  static String buildActionItemsMarkdown(Meeting meeting, Summary? summary) {
    ...
    final buf = StringBuffer();
    buf.writeln('# ${_markdownText(meeting.title)} ${tr('액션 아이템', 'Action items')}');
    buf.writeln();
    if (items.isEmpty) {
      buf.writeln('- ${tr('액션 아이템이 없습니다.', 'No action items.')}');
      return buf.toString();
    }
    for (final item in items) {
      ...
      buf.writeln(
        '- [$done] ${_markdownText(item.task)} _(${tr('담당', 'Owner')}: ${_markdownText(owner)} / ${tr('기한', 'Due')}: ${_markdownText(deadline)})_',
      );
    }
    return buf.toString();
  }
  ...
  static String buildEmailBody(Meeting meeting, Summary? summary) {
    ...
    final dt = meeting.createdAt;
    buf.writeln(
      '${tr('날짜', 'Date')}: ${dt.year}-${dt.month.toString().padLeft(2, '0')}-${dt.day.toString().padLeft(2, '0')}',
    );
    buf.writeln();

    if (summary == null) {
      buf.writeln('(${tr('요약 없음', 'No summary')})');
      return buf.toString();
    }

    if (summary.participants.isNotEmpty) {
      buf.writeln('${tr('참석자', 'Participants')}: ${summary.participants.join(', ')}');
      buf.writeln();
    }
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        header: (ctx) => pw.Column(
          ...
          children: [
            pw.Text(tr('온디바이스 AI 회의록', 'On-device AI meeting minutes'), style: smallStyle),
        ...
        build: (ctx) => [
          ...
          if (transcripts.isNotEmpty) ...[
            ...
            pw.Text(tr('전사본', 'Transcript'), style: sectionStyle),
            pw.SizedBox(height: 6),
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        header: (ctx) => pw.Column(
          crossAxisAlignment: pw.CrossAxisAlignment.start,
          children: [
            pw.Text(tr('회의 보고서', 'Meeting report'), style: smallStyle),
            pw.Divider(thickness: 0.5, color: PdfColors.grey400),
          ],
        ),
        footer: (ctx) => pw.Align(
        ...
        build: (ctx) => [
          pw.Text('${meeting.title} ${tr('보고', 'Report')}', style: titleStyle),
          pw.SizedBox(height: 4),
          _buildDateLine(meeting, baseStyle),
          if (summary?.participants.isNotEmpty == true) ...[
            pw.SizedBox(height: 3),
            pw.Text(
              '${tr('참석자', 'Participants')}: ${summary!.participants.join(', ')}',
              style: baseStyle,
            ),
          ],
          pw.SizedBox(height: 16),
          if (summary == null)
            pw.Text(tr('요약이 아직 생성되지 않았습니다.', 'No summary has been generated yet.'), style: baseStyle)
          else
  ...
  static Future<String?> saveAsTxt(
    ...
    final location = await getSaveLocation(
      acceptedTypeGroups: [
        XTypeGroup(label: tr('텍스트', 'Text'), extensions: const ['txt']),
      ],
  ...
  static Future<String?> saveAsBusinessReportPdf(
    ...
    final safeName = _safeFilename('${meeting.title}_${tr('보고서', 'report')}');
  ...
  static Future<String?> saveAsDocx(
    ...
    final location = await getSaveLocation(
      acceptedTypeGroups: [
        XTypeGroup(label: tr('Word 문서', 'Word document'), extensions: const ['docx']),
      ],
  ...
  static List<pw.Widget> _buildSummaryWidgets(
    ...
    if (s.participants.isNotEmpty) {
      widgets.add(pw.Text(tr('참석자', 'Participants'), style: sectionStyle));
      widgets.add(pw.SizedBox(height: 4));
      widgets.add(
        pw.Padding(
    ...
    addSection(tr('주요 논의', 'Key discussions'), s.keyDiscussions);
    addSection(tr('결정 사항', 'Decisions'), s.decisions);
    ...
    try {
      final items = (jsonDecode(s.actionItemsJson) as List)
          ...
          .toList();
      if (items.isNotEmpty) {
        widgets.add(pw.Text(tr('액션 아이템', 'Action items'), style: sectionStyle));
        widgets.add(pw.SizedBox(height: 4));
        for (final a in items) {
          widgets.add(
            pw.Padding(
    ...
    addSection(tr('미해결 이슈', 'Open questions'), s.openQuestions);
  ...
  static List<pw.Widget> _buildBusinessReportWidgets(
    ...
    addSection(tr('핵심 논의', 'Key discussions'), s.keyDiscussions.take(5).toList());
    addSection(tr('결정 사항', 'Decisions'), s.decisions);
    ...
    addSection(tr('리스크 / 확인 필요', 'Risks / Needs review'), s.openQuestions);
    return widgets;
  }
  ...
  static String _buildDocxDocumentXml(
    ...
    body.write(
      _docxParagraph(
        reportStyle ? '${meeting.title} ${tr('보고', 'Report')}' : meeting.title,
        style: 'Title',
      ),
    );
    body.write(_docxParagraph('${tr('날짜', 'Date')}: $dtStr'));
    if (meeting.durationSeconds > 0) {
      body.write(
        _docxParagraph(
          '${tr('소요 시간', 'Duration')}: ${meeting.durationSeconds ~/ 60}${tr('분', 'm')} ${meeting.durationSeconds % 60}${tr('초', 's')}',
    ...
    if (summary == null) {
      body.write(_docxParagraph(tr('요약이 아직 생성되지 않았습니다.', 'No summary has been generated yet.')));
    } else if (reportStyle) {
      if (summary.participants.isNotEmpty) {
        body.write(_docxParagraph('${tr('참석자', 'Participants')}: ${summary.participants.join(', ')}'));
      }
      _writeDocxList(body, tr('핵심 논의', 'Key discussions'), summary.keyDiscussions.take(5).toList());
      _writeDocxList(body, tr('결정 사항', 'Decisions'), summary.decisions);
      _writeDocxActions(body, summary);
      _writeDocxList(body, tr('리스크 / 확인 필요', 'Risks / Needs review'), summary.openQuestions);
    } else {
      if (summary.participants.isNotEmpty) {
        body.write(_docxHeading(tr('참석자', 'Participants')));
        body.write(_docxParagraph(summary.participants.join(', ')));
      }
      _writeDocxList(body, tr('주요 논의', 'Key discussions'), summary.keyDiscussions);
      _writeDocxList(body, tr('결정 사항', 'Decisions'), summary.decisions);
      _writeDocxActions(body, summary);
      _writeDocxList(body, tr('미해결 이슈', 'Open questions'), summary.openQuestions);
    }

    if (transcripts.isNotEmpty) {
      body.write(_docxHeading(tr('전사본', 'Transcript')));
      for (final seg in transcripts) {
        final start = _secToStr(seg.startTimeSeconds);
        final end = _secToStr(seg.endTimeSeconds);
        final speaker = seg.speakerLabel == null ? '' : ' ${seg.speakerLabel}:';
...
''';
  ...
  static void _writeDocxActions(StringBuffer body, Summary summary) {
    ...
    if (items.isEmpty) return;
    body.write(_docxHeading(tr('액션 아이템', 'Action items')));
    for (final item in items) {
      final done = item.completed ? tr('완료', 'Done') : tr('미완료', 'Pending');
      body.write(
        _docxParagraph(
          '• [$done] ${item.task}  ${tr('담당', 'Owner')}: ${_actionOwnerText(item)} / ${tr('기한', 'Due')}: ${_actionDeadlineText(item)}',
        ),
      );
    }
  }
...
''';
  ...
  static void _writeMarkdownActionItems(StringBuffer buf, Summary summary) {
    ...
    buf.writeln('## ${tr('액션 아이템', 'Action items')}');
    buf.writeln();
    buf.writeln('| ${tr('완료', 'Done')} | ${tr('할 일', 'Task')} | ${tr('담당', 'Owner')} | ${tr('기한', 'Due')} |');
    buf.writeln('| --- | --- | --- | --- |');
    for (final item in items) {
      final done = item.completed ? 'x' : ' ';
      buf.writeln(
  ...
  static void _writeNotionActionItems(StringBuffer buf, Summary summary) {
    ...
    for (final item in items) {
      final done = item.completed ? 'x' : ' ';
      buf.writeln(
        '- [$done] ${_markdownText(item.task)} _(${tr('담당', 'Owner')}: ${_markdownText(_actionOwnerText(item))} / ${tr('기한', 'Due')}: ${_markdownText(_actionDeadlineText(item))})_',
      );
    }
    buf.writeln();
  }
  ...
  static String _actionOwnerText(ActionItem item) =>
      item.ownerNeedsConfirmation ? tr('담당자 미확인', 'Owner TBD') : item.owner;

  static String _actionDeadlineText(ActionItem item) =>
      item.deadlineNeedsConfirmation ? tr('기한 미확인', 'Due date TBD') : item.deadline;
  ...
  static String _markdownTableCell(String value) {
    ...
    return clean.isEmpty ? tr('(미언급)', '(Not mentioned)') : clean;
  }
}
