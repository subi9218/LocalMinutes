...
/// Patched version of RecordLinux that implements record_platform_interface 1.5.0.
/// Added: startStream(), fixed hasPermission() signature.
class RecordLinux extends RecordPlatform {
  ...
  // ── PATCHED: added {bool request = true} to match interface 1.5.0 ──────────
  @override
  Future<bool> hasPermission(String recorderId, {bool request = true}) {
    return Future.value(true);
  }

  // ── PATCHED: added startStream to match interface 1.5.0 ────────────────────
  @override
  Future<Stream<Uint8List>> startStream(
      String recorderId, RecordConfig config) {
    // Linux streaming is not supported in this stub.
    // This method is only required to satisfy the interface at compile time.
  ...
  // ── Private helpers (original from 0.7.2) ─────────────────────────────────
  ...
  Future<void> _callFMedia(
    ...
    if (consumeOutput) {
      ...
      if (outStreamCtrl == null) out.stream.listen((_) {});
